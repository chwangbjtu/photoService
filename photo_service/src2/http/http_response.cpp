...
int http_response::pack(char *buf, int &len,string scontent_type,string cache_time)
{
	int pos = 0, num = 0;
	...
	{//no content
        ...
        */
        ...
        string default_header =  "Content-Length: 0\r\n";
        default_header += "Expires: Mon, 26 Jul 1997 05:00:00 GMT\r\n";
		if ( "0" == cache_time)
		{
        	default_header += "Cache-Control: no-cache, must-revalidate\r\n";
	        default_header += "Pragma: no-cache\r\n";
		}
		else
		{
	        default_header += "Cache-Control: max-age=" + cache_time + "\r\n";
			default_header += "Pragma: max-age=" + cache_time + "\r\n";
		}
    	default_header += "Content-type: " + scontent_type + "\r\n";
		default_header += "Connection: Close\r\n\r\n";

        num = snprintf(buf+pos, len-pos, default_header.data());
	...
	{//zip failed, use original content
        ...
        */
        ...
        string default_header =  "Content-Length: %d\r\n";
        default_header += "Expires: Mon, 26 Jul 1997 05:00:00 GMT\r\n";
		if ( "0" == cache_time)
		{
        	default_header += "Cache-Control: no-cache, must-revalidate\r\n";
        ...
        num = snprintf(buf+pos, len-pos, default_header.data(), content_len);
	...
	{//use zipped content
        ...
        */
        ...
        string default_header =  "Content-Encoding: gzip, deflate\r\n";
        default_header += "Content-Length: %d\r\n";
        default_header += "Expires: Mon, 26 Jul 1997 05:00:00 GMT\r\n";
		if ( "0" == cache_time)
        {
        	default_header += "Cache-Control: no-cache, must-revalidate\r\n";
        ...
        num = snprintf(buf+pos, len-pos, default_header.data(), buf_len);
...
int http_response::pack(string &strbuf, string scontent_type, string cache_time, bool b_gzip)
{
	strbuf = "";
	...
	{//no content
        ...
        */
        ...
        num = snprintf(buf, len, default_header.data());
	...
	if(b_gzip)
	{
		...
		{//zip failed, use original content
            ...
            */
            ...
            num = snprintf(buf, len, default_header.data(), content_len);
			if(num < 0)
				return -1;
			strbuf.append(buf, num);
		...
		{//use zipped content
            ...
            */
            ...
            num = snprintf(buf, len, default_header.data(), buf_len);
